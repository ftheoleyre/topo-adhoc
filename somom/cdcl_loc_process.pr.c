/* Process model C form file: cdcl_loc_process.pr.c */
/* Portions of this file copyright 1992-2002 by OPNET Technologies, Inc. */



/* This variable carries the header into the object file */
static const char cdcl_loc_process_pr_c [] = "MIL_3_Tfile_Hdr_ 81A 30A modeler 7 41CEC4DF 41CEC4DF 1 ares-theo-1 Fabrice@Theoleyre 0 0 none none 0 0 none 0 0 0 0 0 0                                                                                                                                                                                                                                                                                                                                                                                                        ";
#include <string.h>
...
// COMMON
#define		LOC_LOW_SPEED					1
#define		LOC_HIGH_SPEED					2
...
//Energy
double		cdcl_loc_sleeping_total	= 0;
...
//					    FLOWS	
//----------------------------------------------------------------------
int 		nb_pk_loc_data_instantaneous				= 0;
int			nb_pk_loc_data_total						= 0;
int			nb_pk_loc_data_retransmission_instantaneous	= 0;
int			nb_pk_loc_data_retransmission_total			= 0;

int 		nb_pk_loc_rreq_instantaneous				= 0;
int			nb_pk_loc_rreq_total						= 0;

int 		nb_pk_loc_rrep_instantaneous				= 0;
int			nb_pk_loc_rrep_total						= 0;
int			nb_pk_loc_rrep_retransmission_instantaneous	= 0;
int			nb_pk_loc_rrep_retransmission_total			= 0;

int 		nb_pk_loc_rup_instantaneous					= 0;
int			nb_pk_loc_rup_total							= 0;

int 		nb_pk_loc_rdel_instantaneous				= 0;
int			nb_pk_loc_rdel_total						= 0;

int 		nb_pk_loc_ack_instantaneous					= 0;
int			nb_pk_loc_ack_total							= 0;
...
// NETWORK
void 	loc_pk_send_delayed(Packet* pkptr , double delay);
void 	loc_pk_send (Packet* pkptr);
void 	loc_pk_retransmit_delayed(Packet* pkptr , double delay);
void 	loc_pk_retransmit (Packet* pkptr);
...
typedef struct
	...
	} cdcl_loc_process_state;

#define pr_state_ptr            		((cdcl_loc_process_state*) SimI_Mod_State_Ptr)
...
#undef FIN_PREAMBLE
#define FIN_PREAMBLE	cdcl_loc_process_state *op_sv_ptr = pr_state_ptr;
...
void send_rreq_pk(int addr_searched , int stat_id){
	...
	pk_dest = op_pk_create_fmt("cdcl_loc_pk_route_request");
...
void forward_route_request(Packet* pk_src){
	...
	//Transmission
	loc_pk_send_delayed(pk_dest , op_dist_uniform(0.002));	
}
...
void send_route_reply(int addr , int stat_id){
	...
	pk_new = op_pk_create_fmt("cdcl_loc_pk_route_reply");
	...
	loc_pk_send_delayed(pk_new , op_dist_uniform(0.003));
...
Packet* generate_route_delete(){
	...
	pkptr = op_pk_create_fmt("cdcl_loc_pk_route_delete");
...
void send_route_update(){
	...
	pk=op_pk_create_fmt("cdcl_loc_pk_route_update");
...
void acknowledges_unicast_packet(int address , int pk_id){
	...
	//Creates the packet
	pkptr = op_pk_create_fmt ("cdcl_loc_pk_ack");
	...
	//Sends it
	loc_pk_send_delayed(pkptr , op_dist_uniform(0.001));
...
Packet* generate_data_pk(int dest_final){
	...
	//INCOMING PACKET 
	pkptr = op_pk_create_fmt ("cdcl_loc_pk_data");
	...
	if (stats_nb_data_pk > MAX_DATA_PK)
		op_sim_end("We have too many data packet to handle the stats", "please increase the value of MAX_DATA_PK","in the header block of cdcl_loc","");
...
void loc_stats_about_control_pk_flows(Packet* pkptr){
	char	msg[50];
	int		option_type;
	int		type;
	...
	switch(option_type)
		{
			case DATA_PK_TYPE :
				nb_pk_loc_data_instantaneous++;
				nb_pk_loc_data_total++;
			break;
			case RREQ_PK_TYPE :
				nb_pk_loc_rreq_instantaneous++;
				nb_pk_loc_rreq_total++;
			break;
			case RREP_PK_TYPE :
				nb_pk_loc_rrep_instantaneous++;
				nb_pk_loc_rrep_total++;
			break;
			case RUPDATE_PK_TYPE :
				nb_pk_loc_rup_instantaneous++;
				nb_pk_loc_rup_total++;
			break;
			case RDELETE_PK_TYPE :
				nb_pk_loc_rdel_instantaneous++;
				nb_pk_loc_rdel_total++;
			break;
			case ACK_APP_PK_TYPE :
				nb_pk_loc_ack_instantaneous++;
				nb_pk_loc_ack_total++;
			break;
			default :
...
//Set some stats about nerwork control flows
//Type = type of contro packet (HELLO, CLUSTER, CDS or OTHER)
void loc_stats_about_control_pk_flows_retransmission(Packet* pkptr){
	char	msg[50];
	int		option_type;
	...
	switch(option_type)
		{
			case DATA_PK_TYPE :
				nb_pk_loc_data_retransmission_instantaneous++;
				nb_pk_loc_data_retransmission_total++;
			break;
			case RREP_PK_TYPE :
				nb_pk_loc_rrep_retransmission_instantaneous++;
				nb_pk_loc_rrep_retransmission_total++;
			break;
			default :
...
void loc_pk_send_delayed(Packet* pkptr , double delay){
	...
	//Stats
	loc_stats_about_control_pk_flows(pkptr);
...
void loc_pk_retransmit_delayed(Packet* pkptr , double delay){
	...
	//Stats
	loc_stats_about_control_pk_flows_retransmission(pkptr);
...
void loc_pk_retransmit (Packet* pkptr){
	loc_pk_retransmit_delayed(pkptr , 0);
}
...
void write_node_stat(){
	...
	if (my_address<10)
		sprintf(str,"results_localization/stats_per_node/results_0%d.txt",my_address);
	if (my_address>=10)
		sprintf(str,"results_localization/stats_per_node/results_%d.txt",my_address);
...
void node_debug(char* message){
	...
	if (my_address<10)
		sprintf(str,"results_localization/details_per_node/results_0%d.txt",my_address);
	if (my_address>=10)
		sprintf(str,"results_localization/details_per_node/results_%d.txt",my_address);
...
void data_debug(char* message){
	...
	sprintf(str,"results_localization/%d_loc_debug_data.txt",begin_name_stats);
...
void rrep_debug(char* message){
	...
	sprintf(str,"results_localization/%d_loc_debug_rrep.txt",begin_name_stats);
...
void rreq_debug(char* message){
	...
	sprintf(str,"results_localization/%d_loc_debug_rreq.txt",begin_name_stats);
	if (DEBUG>1)
		{
			fichier = fopen(str,"a");
			fprintf(fichier,message);
...
void rup_debug(char* message){
	...
	sprintf(str,"results_localization/%d_loc_debug_route_update.txt",begin_name_stats);
...
void ack_debug(char* message){
	...
	sprintf(str,"results_localization/%d_loc_debug_ack.txt",begin_name_stats);
...
void common_debug(char* message){
	...
	sprintf(str,"results_localization/%d_loc_debug_common.txt",begin_name_stats);
...
void rdel_debug(char* message){
	...
	sprintf(str,"results_localization/%d_loc_debug_rdel.txt",begin_name_stats);
...
void paging_debug(char* message){
	...
	sprintf(str,"results_localization/%d_loc_debug_paging.txt",begin_name_stats);
...
void send_paging_req(int dest_final){
	...
	pkptr = op_pk_create_fmt("cdcl_loc_pk_paging_req");
	op_pk_nfd_set(pkptr , "SRC" , 			my_address);
...
void send_paging_rep(int dest_final , short hops){
	...
	pkptr = op_pk_create_fmt("cdcl_loc_pk_paging_rep");
	op_pk_nfd_set(pkptr , "SRC" , 			my_address);
...
void send_paging_del(int dest_final){
	...
	pkptr = op_pk_create_fmt("cdcl_loc_pk_paging_del");
	op_pk_nfd_set(pkptr , "SRC" , 			my_address);
	op_pk_nfd_set(pkptr , "DEST" , 			BROADCAST);
	op_pk_nfd_set(pkptr , "NODE_SEARCHED" , dest_final);
...
void send_paging_ack(Packet* pk_data , int  ap_dest){
	...
	pkptr = op_pk_create_fmt("cdcl_loc_pk_paging_ack");
	op_pk_nfd_set(pkptr , "SRC" , 			my_address);
...
void send_paging_rep_gratuitous(int code1, int code2){
	...
	pkptr = op_pk_create_fmt("cdcl_loc_pk_paging_rep_gratuitous");
	op_pk_nfd_set(pkptr , "SRC" , 			my_address);
	op_pk_nfd_set(pkptr , "DEST" , 			ADDR_MULTICAST_PAGING_MASTER);
...
extern "C" {
#endif
	void cdcl_loc_process (void);
	Compcode cdcl_loc_process_init (void **);
	void cdcl_loc_process_diag (void);
	void cdcl_loc_process_terminate (void);
	void cdcl_loc_process_svar (void *, const char *, char **);
#if defined (__cplusplus)
} /* end of 'extern "C"' */
#endif
...
void
cdcl_loc_process (void)
	{
	int _block_origin = 0;
	FIN (cdcl_loc_process ());
	if (1)
		{


		FSM_ENTER (cdcl_loc_process)

		FSM_BLOCK_SWITCH
			{
			/*---------------------------------------------------------*/
			/** state (Init) enter executives **/
			FSM_STATE_ENTER_UNFORCED_NOLABEL (0, "Init", "cdcl_loc_process () [Init enter execs]")
				{
			...
			FSM_EXIT (1,cdcl_loc_process)


			/** state (Init) exit executives **/
			FSM_STATE_EXIT_UNFORCED (0, "Init", "cdcl_loc_process () [Init exit execs]")
				{
				}
			...
			FSM_STATE_ENTER_FORCED (1, state1_enter_exec, "Init2", "cdcl_loc_process () [Init2 enter execs]")
				{
				char		str[100];
				FILE*		fichier;
				int			i;
				...
				nb_normal_nodes = nb_nodes - _loc_nb_aps;
			...
			FSM_STATE_EXIT_FORCED (1, "Init2", "cdcl_loc_process () [Init2 exit execs]")
				{
				}
			...
			FSM_STATE_ENTER_UNFORCED (2, state2_enter_exec, "idle", "cdcl_loc_process () [idle enter execs]")
				{
				}


			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (5,cdcl_loc_process)


			/** state (idle) exit executives **/
			FSM_STATE_EXIT_UNFORCED (2, "idle", "cdcl_loc_process () [idle exit execs]")
				{
			...
			FSM_STATE_ENTER_FORCED (3, state3_enter_exec, "PK_from_LOWER", "cdcl_loc_process () [PK_from_LOWER enter execs]")
				{
				//PK Fields
				Packet	*pk_src;
				Packet	*pk_data;
			...
			FSM_STATE_EXIT_FORCED (3, "PK_from_LOWER", "cdcl_loc_process () [PK_from_LOWER exit execs]")
				{
				}
			...
			FSM_STATE_ENTER_UNFORCED (4, state4_enter_exec, "DEAD", "cdcl_loc_process () [DEAD enter execs]")
				{
				}


			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (9,cdcl_loc_process)


			/** state (DEAD) exit executives **/
			FSM_STATE_EXIT_UNFORCED (4, "DEAD", "cdcl_loc_process () [DEAD exit execs]")
				{
				}
			...
			FSM_STATE_ENTER_FORCED (5, state5_enter_exec, "DATA", "cdcl_loc_process () [DATA enter execs]")
				{
				int		dest_final;
				int		next_source;
				int		i;
				...
				/*
								...
								if (!is_AP)
									{
										//If extern communication, the relay (FA) is the AP, else a random intern destination
										if (op_dist_uniform (1.0) > intern_com_ratio)
											dest_final	= ADDR_MULTICAST_AP;
										else
								...
								else
									{
										//If it is an extern communication, I choose a random intern node
										if (op_dist_uniform (1.0) >= intern_com_ratio)
											dest_final	= op_dist_uniform (nb_nodes-_loc_nb_aps-1)+_loc_nb_aps+1;
										//If it is an intern communication, I reduce the probability to send a data packet (I am a privileged node)
										else if (op_dist_uniform (1.0) <= 1 / (nb_nodes * nb_connections) )
											dest_final	= op_dist_uniform (nb_nodes-_loc_nb_aps-1)+_loc_nb_aps+1;
										else
			...
			FSM_STATE_EXIT_FORCED (5, "DATA", "cdcl_loc_process () [DATA exit execs]")
				{
				}
			...
			FSM_STATE_ENTER_FORCED (6, state6_enter_exec, "ENDSIM", "cdcl_loc_process () [ENDSIM enter execs]")
				{
				//write_node_stat();
					...
					int		hops_rrep							= 0;
					//SPEED
				   	double	LOC_SPEED_HIGH_MIN 	= 0;
				   	double	LOC_SPEED_HIGH_MAX 	= 0;
				   	double	LOC_SPEED_LOW_MIN 	= 0;
				   	double	LOC_SPEED_LOW_MAX 	= 0;
					int		loc_speed_class		= 0;
				   	//ENERGY
					...
					if (op_ima_obj_attr_exists(op_topo_parent(op_id_self()), "mobility.Speed_Class") == OPC_TRUE)
						{
							op_ima_obj_attr_get(op_topo_parent(op_id_self()), "mobility.Speed_Class", &loc_speed_class);
							switch(loc_speed_class)		
								{
									case LOC_LOW_SPEED :
					...
					cdcl_loc_sleeping_total += cdcl_sleeping->cumul_sleeping_time;
					...
					if (my_stat_id+1 == nb_nodes)
						{
							...
							//-------------------
							//General Parameters
							op_ima_sim_attr_get(OPC_IMA_DOUBLE, "SP_HIGH_MIN", 	&LOC_SPEED_HIGH_MIN); 
							op_ima_sim_attr_get(OPC_IMA_DOUBLE, "SP_HIGH_MAX",	&LOC_SPEED_HIGH_MAX); 
							op_ima_sim_attr_get(OPC_IMA_DOUBLE, "SP_LOW_MIN", 	&LOC_SPEED_LOW_MIN); 
							op_ima_sim_attr_get(OPC_IMA_DOUBLE, "SP_LOW_MAX",	&LOC_SPEED_LOW_MAX); 
							...
							//       DATA
							//-------------------
							sprintf(filename,"results_localization/%d_loc_debug_pk_data.txt",begin_name_stats);		
							data_file = fopen(filename , "w");
							fprintf(data_file,"SRC	|	DEST	|	SIZE	|	HOPS	|	RETRY	|	GENERATION	|	SENT		|	REC		|	DELAY\n");	
				
							for (i=0 ;  i < stats_nb_data_pk ; i++ )
								{
									if (stats_data_pk[i].source > _loc_nb_aps)
										up_nb_data_pk_sent++;
									else
							...
							//	ROUTE REQUEST
							//-------------------
							sprintf(filename,"results_localization/%d_loc_debug_pk_rreq.txt",begin_name_stats);		
							...
							sprintf(filename,"results_localization/%d_loc_results_global.txt",begin_name_stats);		
				...
				//			fprintf(file, "Nb packets per RREQ Sent					:	%f\n",  		(double) stats_nb_pk_for_all_rreq / stats_nb_rreq );
							...
							fprintf(file, "|                                                          ENERGY                                            |\n");
							fprintf(file, " ------------------------------------------------------------------------------------------------------------\n\n");
							fprintf(file, "Average Sleeping Time per node				:	%f\n", 			(double) cdcl_loc_sleeping_total / nb_nodes  );
							fprintf(file, "Average Sleeping Time per authorized node			:	%f\n", 	(double) cdcl_loc_sleeping_total / (nb_nodes * ratio_of_sleeping_nodes) );
							fprintf(file, "Ratio of Sleeping Time for authorized nodes		:	%f\n", 		(double) cdcl_loc_sleeping_total / (nb_nodes * ratio_of_sleeping_nodes * op_sim_time())  );
							fprintf(file, "\n");
							...
							fprintf(file, "Data									:	%f\n", 				(double) nb_pk_loc_data_total							/((op_sim_time()-TIME_INIT_MAC) * nb_nodes));
							fprintf(file, "Data Retransmissions						:	%f\n", 			(double) nb_pk_loc_data_retransmission_total			/((op_sim_time()-TIME_INIT_MAC) * nb_nodes));
							fprintf(file, "Route Request							:	%f\n", 			(double) nb_pk_loc_rreq_total							/((op_sim_time()-TIME_INIT_MAC) * nb_nodes));
							fprintf(file, "Route Reply								:	%f\n", 			(double) nb_pk_loc_rrep_total							/((op_sim_time()-TIME_INIT_MAC) * nb_nodes));
							fprintf(file, "Route Reply Retransmissions					:	%f\n", 		(double) nb_pk_loc_rrep_retransmission_total			/((op_sim_time()-TIME_INIT_MAC) * nb_nodes));
							fprintf(file, "Route Update							:	%f\n", 				(double) nb_pk_loc_rup_total							/((op_sim_time()-TIME_INIT_MAC) * nb_nodes));
							fprintf(file, "Route delete							:	%f\n", 				(double) nb_pk_loc_rdel_total							/((op_sim_time()-TIME_INIT_MAC) * nb_nodes));
							fprintf(file, "Unicast Acks							:	%f\n", 				(double) nb_pk_loc_ack_total					/((op_sim_time()-TIME_INIT_MAC) * nb_nodes));
							fprintf(file, "Total									:	%f\n", 			(double) (nb_pk_loc_data_total+nb_pk_loc_data_retransmission_total+nb_pk_loc_rreq_total+nb_pk_loc_rrep_total+nb_pk_loc_rrep_retransmission_total+nb_pk_loc_rup_total+nb_pk_loc_rdel_total+nb_pk_loc_ack_total)/((op_sim_time()-TIME_INIT_MAC) * nb_nodes));
							fprintf(file, "\n\n");
			...
			FSM_STATE_EXIT_FORCED (6, "ENDSIM", "cdcl_loc_process () [ENDSIM exit execs]")
				{
				}
			...
			FSM_STATE_ENTER_FORCED (7, state7_enter_exec, "RET_UNICAST", "cdcl_loc_process () [RET_UNICAST enter execs]")
				{
									...
									if (couple->timeout <= op_sim_time())
										{
											...
											//if (couple->nb_retry < MAX_NB_RETRIES_UNICAST_ACKS)
											else if (couple->timeout <= op_sim_time())
												{
													...
													loc_pk_retransmit_delayed (op_pk_copy(couple->pkptr) , delay);			
			...
			FSM_STATE_EXIT_FORCED (7, "RET_UNICAST", "cdcl_loc_process () [RET_UNICAST exit execs]")
				{
				}
			...
			FSM_STATE_ENTER_FORCED (8, state8_enter_exec, "SEND_UPDATE", "cdcl_loc_process () [SEND_UPDATE enter execs]")
				{
				/*char 		msg[150];
				double		tmp1, tmp2;
			...
			FSM_STATE_EXIT_FORCED (8, "SEND_UPDATE", "cdcl_loc_process () [SEND_UPDATE exit execs]")
				{
				}
			...
			FSM_STATE_ENTER_FORCED (9, state9_enter_exec, "STATE_CHANGED", "cdcl_loc_process () [STATE_CHANGED enter execs]")
				{
			...
			FSM_STATE_EXIT_FORCED (9, "STATE_CHANGED", "cdcl_loc_process () [STATE_CHANGED exit execs]")
				{
				}
			...
			FSM_STATE_ENTER_FORCED (10, state10_enter_exec, "SON_LOST", "cdcl_loc_process () [SON_LOST enter execs]")
				{
				/////////////////////////////////////////////////
				//
			...
			FSM_STATE_EXIT_FORCED (10, "SON_LOST", "cdcl_loc_process () [SON_LOST exit execs]")
				{
				}
...
Compcode
cdcl_loc_process_init (void ** gen_state_pptr)
	{
	int _block_origin = 0;
	static VosT_Address	obtype = OPC_NIL;

	FIN (cdcl_loc_process_init (gen_state_pptr))

	if (obtype == OPC_NIL)
		{
		/* Initialize memory management */
		if (Vos_Catmem_Register ("proc state vars (cdcl_loc_process)",
			sizeof (cdcl_loc_process_state), Vos_Vnop, &obtype) == VOSC_FAILURE)
			{
	...
	*gen_state_pptr = Vos_Catmem_Alloc (obtype, 1);
	if (*gen_state_pptr == OPC_NIL)
		...
		((cdcl_loc_process_state *)(*gen_state_pptr))->current_block = 0;
...
void
cdcl_loc_process_terminate (void)
	{
	int _block_origin = __LINE__;

	FIN (cdcl_loc_process_terminate (void))
...
/* local variable prs_ptr in cdcl_loc_process_svar function. */
...
void
cdcl_loc_process_svar (void * gen_ptr, const char * var_name, char ** var_p_ptr)
	{
	cdcl_loc_process_state		*prs_ptr;

	FIN (cdcl_loc_process_svar (gen_ptr, var_name, var_p_ptr))
	...
	prs_ptr = (cdcl_loc_process_state *)gen_ptr;
